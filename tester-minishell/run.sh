...
function run_test() {
    ...
    if [[ $VALGRIND -eq 1 ]] && ([[ "USER_EXIT" -eq 69 || $(cat ./user_outputs/valgrind.log | grep "FILE DESCRIPTORS:" | uniq -w 1 | wc -l) -ne 1 ]] || [[ $(cat ./user_outputs/valgrind.log | grep "FILE DESCRIPTORS:" | uniq -w 1 | awk '{print $4}') -ne $VALGRIND_FD_NB ]]); then
        echo -e " ${RED}KO${ENDCOLOR} (Valgrind error)"
        test_passed=false
    elif [ "$(cat user_outputs/err | wc -l)" != "$(cat bash_outputs/err | sed '/bash: line [0-9]: `/d' | wc -l)" ]; then
        echo -e " ${RED}KO${ENDCOLOR} (Error output mismatch)"
        test_passed=false
    elif [ "$BASH_EXIT" != "$USER_EXIT" ]; then
        echo -e " ${RED}KO${ENDCOLOR} (Exit code mismatch)"
        test_passed=false
    else
    ...
    if [ "$test_passed" = false ]; then
        ...
        if [[ $VALGRIND -eq 1 && "$test_passed" = false ]]; then
            echo -e $'\n'${RED}========== ${ENDCOLOR}Valgrind log${RED} ==========${ENDCOLOR}$'\n'
            cat ./user_outputs/valgrind.log
            echo -e ${RED}=====================$(seq 14 | xargs -I{} echo -n =)${ENDCOLOR}$'\n'
        fi
        if [ "$(cat user_outputs/err | wc -l)" != "$(cat bash_outputs/err | sed '/bash: line [0-9]: `/d' | wc -l)" ]; then
            echo "ERROR DIFF:"
            echo "$ERR_DIFF"
        fi
        if [ "$BASH_EXIT" != "$USER_EXIT" ]; then
            echo "EXIT CODE DIFF:"
...
VALGRIND_ARGS=
if [[ $VALGRIND -eq 1 ]]; then
    VALGRIND_ARGS="valgrind --log-file=../user_outputs/valgrind.log --trace-children=yes --track-fds=yes --leak-check=full --show-leak-kinds=all --suppressions=../supress.txt --error-exitcode=69"
